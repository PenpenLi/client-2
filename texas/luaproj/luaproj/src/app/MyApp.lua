...
function MyApp:run()
	...
	self.tid1 = scheduler.scheduleGlobal(function()
		self:CheckVersion("texas", "games/texas/", "http://poker.game577.com/game_update/texas/")
		scheduler.unscheduleGlobal(self.tid1)
	end, 0.1);
end

function MyApp:DoCheckVersion(exe, localp, remotep)
	printLog("a", "DoCheckVersion begin:%s %s %s", exe, localp, remotep);
	coroutine.resume(self.co, exe, localp, remotep);
	local st = coroutine.status(self.co);
	if st == "dead" then
	...
	elseif chk == 5 then
		...
		self.sh1 = scheduler.scheduleGlobal(function()
			...
			elseif self.cor_182331 == 1 then
				printLog("a", "check_version is_working = false")
				--成功
				if st == 7 then
				--仍然存在旧版本
				elseif st == 8 then
