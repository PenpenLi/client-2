...
local SimpleBrowser = class("SimpleBrowser", cc.mvc.ViewCommon)
...
function SimpleBrowser:ctor(options)
    ...
    local csbnode = cc.CSLoader:createNode("common/BrowserLayer.csb")
    csbnode:addTo(self)
    ...
    local function onDisFailLoading(webview, url)
        printLog("a","onDisFailLoading:" .. tostring(url))
    end
	local container = csbnode:getChildByName("container");
