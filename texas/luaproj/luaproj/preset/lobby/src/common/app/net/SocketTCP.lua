...
function SocketTCP:connect(__host, __port, __retryConnectWhenFailure)
	...
	printInfo("%s.connect(%s, %s)", self.name, self.host, self.port)
	-- self.tcp = socket.tcp()

	local isipv6_only = false
	printLog("a","----- ", self.host..":"..self.port)
	...
	printLog("a","isipv6_only", isipv6_only)
	dump(addrinfo)
	if isipv6_only then
...
function SocketTCP:_onConnected()
	...
	local __tick = function()
		-- printInfo("%s.tick1", self.name)
		while true do
			-- printInfo("%s.tick2", self.name)
			...
			-- printLog("a","body:", __body, "__status:", __status, "__partial:", __partial)
    	    if __status == STATUS_CLOSED or __status == STATUS_NOT_CONNECTED then
				-- printInfo("%s.tick close", self.name)
		    	self:close()
		    	-- modified by Helay
