...
--		:align(display.LEFT_BOTTOM, 0, -4)
    ...
    self:schedule(handler(self, self.handleMessage), 0.1, "BroadcastNode.handlerBroadcast")
    self:setOpacity(0)
end

function BroadcastNode:onEnter()
	printLog("a","BroadcastNode:onEnter()")
...
function BroadcastNode:actionShow()
	printLog("a","BroadcastNode:actionShow")
	self:stopAllActions()
	self:runAction(
...
function BroadcastNode:actionHide()
	printLog("a","BroadcastNode:actionHide")
	self:stopAllActions()
	self:runAction(
		cc.EaseSineOut:create(
