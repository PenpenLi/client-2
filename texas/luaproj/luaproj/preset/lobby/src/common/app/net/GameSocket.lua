...
function GameSocket:isWorking() 
	return self._status == GameSocket.S_CONNECTED or self._status == GameSocket.S_CONNECTING;
end
...
function GameSocket:onSocketEvent(event)
    ...
    elseif event.name == SocketTCP.EVENT_CONNECT_FAILURE then
        ...
        self:switchStatus(GameSocket.DISCONNECTED)
		self:close()
