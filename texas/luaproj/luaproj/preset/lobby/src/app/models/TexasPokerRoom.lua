local TexasPokerRoom = class("TexasPokerRoom")

function TexasPokerRoom:ctor()
	self.id = 0
	self.balance_with = 0
...
function TexasPokerRoom:setServerData(data)
...
function TexasPokerRoom:changeStatus(data)
...
function TexasPokerRoom:changeSidePool( index, value)
    self.pools[index] = value
end

-- 优先使用id为2的当做主池
function TexasPokerRoom:getMainPool()
...
function TexasPokerRoom:updatePublicCards(data)
	self.cards = data.cards_
end

function TexasPokerRoom:clearPublicCards()
	self.cards = ""
end

function TexasPokerRoom:updateBanker(data)
	self.banker_set = data.uid_
end

function TexasPokerRoom:updateMaxBet(bet)
	self.max_bet = bet
end

function TexasPokerRoom:clear()
	self.status = 0			-- 状态
	self.time_left = 0 		-- 本状态剩余时间
	self.time_total = 0 	-- 本状态总时间
    self.cards = ""         -- 公共牌
