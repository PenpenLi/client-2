...
--	self.text_MainPool = UIHelper.seekNodeByName(self.csbnode, "AtlasLabel_Pool_1")
	...
	self.evt1312 = addListener(self, "NET_MSG", handler(self, self.onMsg));
end
...
function ViewRoom:onMsg(fromServer, subCmd, content)
	...
	elseif subCmd == CMD.GAME_ROOM_DESDROY then
		if content.reason_ == "0" then
			APP:getCurrentController():showAlertOK({desc = LANG.TIP_ROOM_DESTROYED_DISMISS});
		else
			APP:getCurrentController():showAlertOK({desc = LANG.TIP_ROOM_DESTROYED_EXPIRED});
		end
		self:clear();
		APP:getCurrentController():showLastGameLayer();
	end
...
function ViewRoom:onPrivateInfo(content)
	...
	if not self.evt131r3 then
		self.evt131r3 = scheduler.scheduleGlobal(handler(self, self.updateTimeLeft), 0.4)
	end
...
function ViewRoom:leaveSeat(serverpos, why)
	if serverpos < 0 then
		--如果变为观察者
		...
		if player and player.uid == APP.GD.GameUser.uid then
			...
			if APP.GD.GameUser.is_observer_ == 1 then
				...
				APP.GD.room_id = 0;
				APP:ActiveCtrl("HomeController", why == 4);
			end
		end
	end
...
function ViewRoom:onExit()
	...
	removeListener(self.evt1312);
	if self.evt131r3 then
		scheduler.unscheduleGlobal(self.evt131r3)
	end
end
