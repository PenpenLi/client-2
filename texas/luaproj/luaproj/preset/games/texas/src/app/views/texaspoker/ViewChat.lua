...
function ViewChat:sendPreChat(idx)
    printLog("a","sendPreChat"..self.PreChatMsgData[idx])
    local content = APP.GD.LANG.CHAT_MSG[idx]
    APP:getCurrentController():chatRequest(content)
    self:actionExit()
end 

function ViewChat:sendFace(idx)
    printLog("a","sendFace"..string.format("face_default_%d.png",idx))
