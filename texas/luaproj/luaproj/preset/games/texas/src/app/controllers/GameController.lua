...
function GameController:getPlayer(uid)
    local players = APP.GD.room_players
    return players:getPlayerByUid(uid)
end

function GameController:getMe()
    local gameUser =APP.GD.GameUser
    return self:getPlayer(gameUser.uid);
end

function GameController:showAddBetLayer(mul)
    local room = APP.GD.game_room;
...
function GameController:addViewPlayer(player, init)
...
----------------------------------游戏相关---------------------------------------------------------
function GameController:changeStatus()
    printLog("a","GameController:changeStatus")
    local room = APP.GD.game_room
    local players = APP.GD.room_players
...
function GameController:newTurnStart()
	local room = APP.GD.game_room
    local players = APP.GD.room_players
...
--玩家被选定为庄家
function GameController:promoteBanker( content )
    printLog("a","GameController:promoteBanker")
...
--要求玩家采取操作
function GameController:pleaseTakeop( content )
    printLog("a","要求玩家采取操作GameController:pleaseTakeop")
...
function GameController:playerCards( content )
    printLog("a","GameController:playerCards")
...
--公共牌
function GameController:publicCards( content )
    printLog("a","GameController:publicCards")
    local cards_str = string.split(content.cards_, ",")
    local cards = {}
    for i = 1, #cards_str do
        table.insert(cards, tonumber(cards_str[i]))
...
--广播玩家采取了什么操作
function GameController:playerOption( content )
    printLog("a","GameController:playerOption")
...
--最佳牌型
function GameController:bestPlan(uid, content)
    printLog("a","GameController:bestPlan")
...
-- 底池变化
-- 这里id为1和2时要注意
function GameController:poolChange(pool_id)
    printLog("a","GameController:poolChange")
...
--比赛结果
function GameController:matchResult(content)
    printLog("a","GameController:matchResult")
    local players = APP.GD.room_players
    local room = APP.GD.game_room
    local gameUser =APP.GD.GameUser
...
-- 更新金币
function GameController:updatePlayerGold(uid, gold)
...
--玩家坐到位置
function GameController:playerSeat( uid )
	local player = APP.GD.room_players:getPlayerByUid(uid);
    self:addViewPlayer(player)
end

--玩家离开房间
function GameController:playerLeave( delPlayer, why )
	if not delPlayer then
		self.viewRoom:leaveSeat(-1, why);
	else
		local player = self.playerViews[delPlayer.uid]
		self.viewRoom:leaveSeat(delPlayer.pos, why);
		self.playerViews[delPlayer.uid] = nil;
	end
end

function GameController:sitDown(clipos)
